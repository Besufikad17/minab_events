  ...
  <client-only>
    ...
    <div class="flex flex-col items-center mt-20 mb-20">
      ...
      <div class="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-2 mx-auto">
        <ImageCard v-for="(category, index) in categories" :key="index" :title="category.name" :image="`url(${category.image})`" :link="`/home?category=${category.name}`" />
      </div>
    </div>
  </client-only>
  
  <div class="flex flex-col items-center mt-20 mb-20">
    <h2 class="text-4xl font-extrabold dark:text-white">Features</h2><br/>
